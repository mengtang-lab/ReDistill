...
def get_activations(
    files, model, batch_size=50, img_size=32, dims=2048, device="cpu", num_workers=1
):
    ...
    dataset = ImagePathDataset(files, transforms=tran_transform)
...
def compute_statistics_of_path(path, model, batch_size, img_size, dims, device, num_workers=1):
    if path.endswith(".npz"):
        ...
        m, s = calculate_activation_statistics(
            files, model, batch_size, img_size, dims, device, num_workers
        )
...
def calculate_fid_given_paths(paths, batch_size, img_size, device, dims, num_workers=1):
    ...
    m2, s2 = compute_statistics_of_path(
        paths[1], model, batch_size, img_size, dims, device, num_workers
    )
...
def main():
    ...
    fid_value = calculate_fid_given_paths(
        args.path, args.batch_size, args.img_size, device, args.dims, num_workers
    )
    print("FID: ", fid_value)
