...
try:
    ...
    pass
from diffusion_reed import GaussianDiffusionTrainer, GaussianDiffusionSampler, GaussianDiffusionSamplerDDIM
from model.model import UNet, CelebAHF_UNet
...
#DISTILLATION
flags.DEFINE_bool('distill', True, help='use distillation')
flags.DEFINE_string('teacher_pretrain', './', help='pretrained UNet teacher')
# flags.DEFINE_string('student_pretrain', './', help='pretrained UNet student')
...
flags.DEFINE_string('sample_method', 'uncond', help='sampling method, must be in [ddim, ddpm / cfg, uncond]')
...
def train():
    ...
    net_model, ema_model = create_model()
    ...
    with trange(0, FLAGS.total_steps, dynamic_ncols=True) as pbar:
        for step in pbar:
            ...
            x_0 = x_0.to(device)
            y_0 = y_0.to(device)
            loss_ddpm, loss_distill = trainer(x_0, y_0)
            loss_ddpm = loss_ddpm.mean()

            loss = loss_ddpm + loss_distill
            loss.backward()
            ...
            writer.add_scalar('loss_distill', loss_distill, step)
            pbar.set_postfix(loss='%.5f' % loss,
                             loss_ddpm='%.5f' % loss_ddpm,
                             loss_distill='%.5f' % loss_distill)

            # sample
            if step != 0 and step % FLAGS.sample_step == 0:
                net_model.eval()
                with torch.no_grad():
                    x_0, _  = ema_sampler(fixed_x_T, method=FLAGS.sample_method)
...
def create_model():
    ...
    if FLAGS.distill:
        if FLAGS.data_type.lower() == "celebahq":
            teacher_model = CelebAHF_UNet().to("cpu")
        else:
            # model setup
            teacher_model = UNet(
                T=FLAGS.T, ch=FLAGS.ch, ch_mult=FLAGS.ch_mult, attn=FLAGS.attn,
                num_res_blocks=FLAGS.num_res_blocks, dropout=FLAGS.dropout,
                cond=FLAGS.conditional, num_class=FLAGS.num_class, pool_strides=[1, 2, 2, 2])
        ...
        net_model = ReED_Diffuser(student_model, [], './config/no_dist_config.yaml', dummy_input={'x': torch.randn(1, 3, FLAGS.img_size, FLAGS.img_size),
                                                                                                           't': torch.randint(FLAGS.T, size=(1, ))})
    # ema_model = copy.deepcopy(net_model)
    # ema_model = ReED_DeepCopy(net_model)
    # print(net_model)
    return net_model, net_model
