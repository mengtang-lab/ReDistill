...
class ReedTrainer(BaseTrainer):
    def init_optimizer(self, cfg):
        if cfg.SOLVER.TYPE == "SGD":
            optimizer = optim.SGD(
                filter(lambda p: p.requires_grad, self.distiller.parameters()),
                lr=cfg.SOLVER.LR,
                momentum=cfg.SOLVER.MOMENTUM,
                weight_decay=cfg.SOLVER.WEIGHT_DECAY,
            )
    ...
    def train_iter(self, data, epoch, train_meters):
        ...
        # forward
        (preds, _), dist_loss = self.distiller(image)
        task_loss = F.cross_entropy(preds, target)
        alpha = 0.5
        loss = (1 - alpha) * task_loss + alpha * dist_loss
        # print(task_loss, dist_loss, loss)
