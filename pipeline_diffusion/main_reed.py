...
from diffusion_reed import GaussianDiffusionTrainer, GaussianDiffusionSampler
...
flags.DEFINE_string('dataset_root', '', help='dataset directiory')
# DISTILLATION
flags.DEFINE_string('teacher_pretrain', './ckpt/diffuser-unet-1222.pt', help='pretrained UNet teacher')
# flags.DEFINE_string('student_pretrain', './ckpt/diffuser-unet-2221.pt', help='pretrained UNet student')
flags.DEFINE_string('dist_config', './config/dist_config.yaml', help='red distillation config')
flags.DEFINE_float('alpha', 0.5, help='distillation loss ratio')
# UNet
flags.DEFINE_integer('ch', 128, help='base channel of UNet')
flags.DEFINE_multi_integer('ch_mult', [1, 2, 2, 2], help='channel multiplier')
flags.DEFINE_multi_integer('pool_strides', [4, 1, 2, 1], help='pooling strides')
...
def train():
    ...
    if FLAGS.dataset=='cifar10':
        ...
        class CelebA(Dataset):
            def __init__(self, csv_file, root_dir, transform=None):
                ...
                self.landmarks_frame = self.landmarks_frame[self.landmarks_frame['partition'] == 0]
        ...
        dataset = CelebA(csv_file=f'{FLAGS.dataset_root}/list_eval_partition.csv', root_dir=f'{FLAGS.dataset_root}/img_align_celeba/img_align_celeba/',
                         transform=transforms.Compose([
    ...
    #     dataset = ImageNet(
    #         root='/mnt/data/downloads/imagenet2012/', split='train',
    #         transform=transforms.Compose([
    #             transforms.RandomHorizontalFlip(),
    #             transforms.ToTensor(),
    ...
    #         num_workers=FLAGS.num_workers, drop_last=True)
        ...
        raise NotImplementedError(f"Unsupported dataset: {FLAGS.dataset}!")
    ...
    with trange(FLAGS.total_steps, dynamic_ncols=True) as pbar:
        for step in pbar:
            ...
            diff_loss, dist_loss = trainer(x_0)
            # print(diff_loss.mean(), dist_loss)

            loss = (1-FLAGS.alpha) * diff_loss.mean() + FLAGS.alpha * dist_loss
            loss.backward()
...
def create_model():
    ...
    student_model = UNet(
        T=FLAGS.T, ch=FLAGS.ch, ch_mult=FLAGS.ch_mult, attn=FLAGS.attn,
        num_res_blocks=FLAGS.num_res_blocks, dropout=FLAGS.dropout, pool_strides=FLAGS.pool_strides)
    # if FLAGS.student_pretrain:
    #     student_ckpt = torch.load(FLAGS.student_pretrain)
    #     # student_model.load_state_dict(student_ckpt['net_model'])
    #     student_model.load_state_dict(student_ckpt['ema_model'])
    ...
    net_model = ReED_Diffuser(student_model, [teacher_model], FLAGS.dist_config, dummy_input={'x': torch.randn(1, 3, FLAGS.img_size, FLAGS.img_size),
                                                                                              't': torch.randint(FLAGS.T, size=(1, ))})

    # ema_model = copy.deepcopy(net_model)
    # ema_model = ReED_DeepCopy(net_model)
    return net_model
